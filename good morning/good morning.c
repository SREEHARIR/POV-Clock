...
void elay_us(int n)       //Delay function for variable time delay
{
    while (n--)
    {
         delay_us(1);
...
int time()            //Calculating time for each revolution
{
    int t=0;
    if(PORTA.F0==0x00)
    {
...
void show(int image[],int t)      //Displaying the letter
{
     int b1=0;
     for(b1=0;b1<=len;b1++)
     {
...
void main()
{
    ...
    UART1_Init(9600);
    delay_us(100);             //Delay for UART to become stable
    while(1)
    {
        //ti=time();
            if (UART1_Data_Ready())    // Bluetooth data recieve
            {
                ch[size1] = UART1_Read();
                ch[size1+1] = '\0';
                size1++;
        ...
        while(ch[size1]!='\0')
        {
             size1++;                      //size of string
        }
             */
        /*for(i=size1;i<size;i++)           //appending space at the end
        {
             ch[i]=' ';
        }*/
        t=30;                   //calculating time delay
        /*
        ...
        else
        {
            for(i=0;i<20;i++)
            {
                 switch(ch[i])             //Identifing the letter
                 {
                     case 'a':
                     case 'A':
                     {
